...
var apiKey = 'a99Gd4OhC6yIPBv9RbwQVW9n3Py2rquc';
...
function updateBestSellers(nytimesBestSellers) {
	...
	function updateCover(id, isbn) {
		fetch('https://www.googleapis.com/books/v1/volumes?q=isbn:' + isbn + "&key=AIzaSyDdK02oLmgnjSB2CUe3AFNBUcEZ0inS9Rs", {
		method: 'get'
		})
		.then(response => { return response.json();})
		.then(data => {
		var img = data.items[0].volumeInfo.imageLinks.thumbnail;
		img = img.replace(/^http:\/\//i, 'https://'); //swap http to https
		$('#cover-' + id).attr('src', img);
		})
	  .catch(error => {
	    console.log(error);
	    console.log('Googel API Error: Defaulting to archival images for book #' + id + ' ISBN: ' + isbn);
	    var img = 'https://via.placeholder.com/128x193';
	    $('#cover-' + id).attr('src', img);
	  });
	}
// https://www.googleapis.com/books/v1/volumes?q=isbn:0735219117&key=AIzaSyDdK02oLmgnjSB2CUe3AFNBUcEZ0inS9Rs